...
|---|---|---|---|---|
| 0 | Loop follow-on from the replay gate: fill ingestion from broker executions into book state, no re-send while an order is working, **cancel the bracket's working orders at the 15:45 flatten (new, 2026-09-06: an entry now goes out as a parent plus a resting stop child and nothing cancels either, so a live account would be left with a stop for a position that no longer exists and an unfilled entry that could fill on the next open)**, alerts from the loop, Gateway outage handled without a false halt, halts clearable, orphan positions alert, 10197 and delayed data visible, halt reason bounded. The sector, halt and cross-book fields are DONE (7937e78) | Gate found nine bugs; no book can trade until fixed | operational | queued behind Momentum v2 |
