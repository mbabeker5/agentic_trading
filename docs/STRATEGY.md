...
| Per symbol cap, whole account | 15% of the money the books hold between them (A9, 2026-09-06) | Across all five books, not just this one. Since the hub retired one ticker one book on 2026-09-06 this is the rule that stops the books piling into a single name |
...
| Books sharing a ticker (hub, 2026-09-06) | Allowed | Two books MAY hold the same name at the same time. One ticker, one book was the rule until 2026-09-06 and the hub retired it: the order reference tag plus each book's own record already say whose shares are whose, and forbidding it deleted the one signal month one exists to measure, which is two strategies agreeing on a name. What replaced it is arithmetic: for every ticker, the broker's netted line has to equal the sum of what every book believes it holds, and when it does not, every book holding that ticker is halted and no other book is touched. The rule id `symbol_exclusive` still runs and reports which other book is in a name, it just no longer refuses. Mo can overturn this; the reasoning is in `/Users/mtalib/workspace_repos/personal_repo/agentic_trading/journal/2026-09-06.md` |
