-- schema.sql
--
-- The whole system of record in one readable piece: every table, every index,
-- every check, with the plain English reason each one exists.
--
-- THIS FILE IS NOT WHAT RUNS. The database is built by applying the files in
-- data/migrations/ in name order, and scripts/migrate_db.py is what applies
-- them. This is the snapshot a person reads when they want to know what is in
-- there without piecing five migrations together in their head.
--
-- The two cannot drift apart without someone noticing: tests/test_db.py builds
-- one database from the migrations and another from this file, and fails if
-- the resulting tables and indexes are not identical. So when you add a
-- migration, paste its statements in here too, in the right section.
--
-- Where the database lives: data/trading.sqlite under the project root, the
-- folder agent/paths.py works out. On this machine today that is
-- /Users/mtalib/workspace_repos/personal_repo/agentic_trading/data/trading.sqlite
-- The whole data/ folder is gitignored apart from this file and the
-- migrations, because a database is state, not source.
